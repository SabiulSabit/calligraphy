...
export default class RegistrationForm {
    ...
    emailAfterDelay() {
        ...
        if (!this.email.errors) {
            axios.post('/doseEmailExist', {_csrf:this._csrf, email: this.email.value })
                .then((response) => {
                    if (response.data) {
                        this.email.isUnique = false;
    ...
    usernameImmediately() {
        if (this.username.value != "" && !/^([a-zA-Z0-9]+)$/.test(this.username.value)) {
            this.showValidationError(this.username, "User name can only contain letters and numbers")
        }
    ...
    usernameAfterDelay() {
        if (this.username.value.length < 3) {
            this.showValidationError(this.username, "Usrename must be at least 3 characters.")
        }

        if (!this.username.errors) {
            axios.post("/doseUserNameExist", {_csrf:this._csrf, username: this.username.value })
                .then((response) => {
                    if (response.data) {
                        this.showValidationError(this.username, "User name already exist")
                        this.username.isUnique = flase
                    }
                    else {
